...
def save_and_maybe_display(
    optimizing_img, dump_path, config, img_id, num_of_iterations
):
    """
...
def neural_style_transfer(config):
    ...
    optimizer = LBFGS(
        (optimizing_img,), max_iter=num_of_iterations, line_search_fn="strong_wolfe"
    )
    cnt = 0
    ...
    def closure():
        ...
        with torch.no_grad():
            ...
            save_and_maybe_display(
                optimizing_img, dump_path, config, cnt, num_of_iterations
            )
        cnt += 1
        return total_loss
